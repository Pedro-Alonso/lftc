...
export const useAutomaton = (): IAutomatonPage => {
  ...
  const onTestWord = useCallback(() => {
    // Algoritmo de teste de palavra em autômato finito
    const initialState = automaton.states.find((state) => state.isInitial);
    if (!initialState) {
      setTestResult({
        accepted: false,
        message: "Defina um estado inicial!",
      });
      return;
    }
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      for (const connection of automaton.connections) {
        if (connection.sourceId === currentState) {
          const symbols = connection.label.split("|");
          if (symbols.includes(char)) {
            currentState = connection.targetId;
            found = true;
            break;
          }
        }
      ...
      if (!found) {
        setTestResult({
          accepted: false,
          message: `Rejeitada: Não há transição do estado para o símbolo '${char}'!`,
        });
        return;
      }
    }

    const finalState = automaton.states.find(
      (state) => state.id === currentState
    );
    if (finalState?.isFinal) {
      setTestResult({ accepted: true, message: "Palavra aceita!" });
    } else {
      setTestResult({
        accepted: false,
        message: "Rejeitada: Estado final não é de aceitação!",
      });
    }
